...
function removeProductFromCart(productId) {
  // Write your code here
  const removeITem = SHOPPING_CART.find((item) => item.id == productId);
  if(!removeITem) return;
  const productRemove = SHOPPING_CART.filter((item) => item.id !==productId);
  SHOPPING_CART.length = 0;
  SHOPPING_CART.push(...productRemove);
